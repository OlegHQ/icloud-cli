...
use crate::ast::types::{
    ...
    CaseModificationOp, DefaultValueOp, DoubleQuotedPart, ErrorIfUnsetOp, GlobPart,
    InnerParameterOperation, LengthOp, LengthSliceErrorOp, ParameterExpansionPart,
    ParameterOperation, PatternAnchor, PatternRemovalOp, PatternRemovalSide, PatternReplacementOp,
    SubstringOp, TildeExpansionPart, TransformOp, TransformOperator, UseAlternativeOp, WordNode,
    WordPart, AST,
};
...
fn parse_parameter_operation(
    ...
    if char == ':' {
        ...
        if "-=?+".contains(op) {
            ...
            let word_parts = parse_word_parts(
                ctx, &word_str, false, false, true, // isAssignment=true for tilde expansion
                false, quoted, false, false, true, // inParameterExpansion
            );
    ...
    if "-=?+".contains(char) {
        ...
        let word_str: String = chars[i..word_end].iter().collect();
        let word_parts = parse_word_parts(
            ctx, &word_str, false, false, true, false, quoted, false, false, true,
        );
        let word = AST::word(if word_parts.is_empty() {
            vec![AST::literal("")]
        } else {
    ...
    if char == '#' || char == '%' {
        ...
        let pattern_parts = parse_word_parts(
            ctx,
            &pattern_str,
            false,
            false,
    ...
    if char == '/' {
        ...
        let pattern_end =
            if anchor.is_some() && (chars.get(i) == Some(&'/') || chars.get(i) == Some(&'}')) {
                i // Pattern is empty
            } else {
        ...
        if chars.get(pattern_end) == Some(&'/') {
            ...
            let replace_parts = parse_word_parts(
                ctx,
                &replace_str,
                false,
                false,
    ...
    if char == '^' || char == ',' {
        ...
        return Ok((
            Some(InnerParameterOperation::CaseModification(
                CaseModificationOp {
                    direction,
                    all,
                    pattern,

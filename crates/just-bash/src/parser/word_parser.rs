...
fn decode_utf8_with_recovery(bytes: &[u8]) -> String {
    ...
    while i < bytes.len() {
        ...
        if (b0 & 0xe0) == 0xc0 {
            if i + 1 < bytes.len() && (bytes[i + 1] & 0xc0) == 0x80 && b0 >= 0xc2
                ...
                let brace_items: Vec<String> = items
                    ...
                    .map(|item| match item {
                        BraceItem::Range {
                            ...
                            let start_val =
                                start_str.clone().unwrap_or_else(|| format!("{}", start));
...
mod tests {
    ...
    fn test_decode_utf8_with_recovery() {
        // Valid UTF-8
        assert_eq!(
            decode_utf8_with_recovery(&[0x48, 0x65, 0x6c, 0x6c, 0x6f]),
            "Hello"
        );

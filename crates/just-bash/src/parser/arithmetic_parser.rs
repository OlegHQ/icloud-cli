...
use super::arithmetic_primaries::{
    parse_ansi_c_quoting, parse_arith_number, parse_localization_quoting, parse_nested_arithmetic,
    skip_arith_whitespace, ARITH_ASSIGN_OPS,
};
use crate::ast::types::*;
...
fn parse_arith_postfix(input: &str, pos: usize) -> (ArithExpr, usize) {
    ...
    if matches!(
        &expr,
        ArithExpr::Concat(_) | ArithExpr::Variable(_) | ArithExpr::DynamicElement(_)
    ) {
        for op_str in ARITH_ASSIGN_OPS {
            if input[current_pos..].starts_with(op_str) && !input[current_pos..].starts_with("==") {
...
fn parse_arith_primary(input: &str, pos: usize, skip_assignment: bool) -> (ArithExpr, usize) {
    ...
    if let Some(result) =
        parse_nested_arithmetic(|s, p| Some(parse_arith_expr(s, p)), input, current_pos)
    {
    ...
    if current_pos < chars.len() && chars[current_pos].is_ascii_digit() {
        ...
        if current_pos < chars.len()
            && (chars[current_pos].is_ascii_alphabetic() || chars[current_pos] == '_')
        {
        ...
        if current_pos < chars.len() && chars[current_pos] == '[' {
            let error_token: String = chars[current_pos..]
                .iter()
                .collect::<String>()
                .trim()
                .to_string();
    ...
    if input[current_pos..].starts_with("${") {
        ...
        if after_brace < chars.len() && chars[after_brace] == '#' {
            ...
            while value_end < chars.len()
                && (chars[value_end].is_ascii_alphanumeric()
                    || chars[value_end] == '@'
                    || chars[value_end] == '_')
            {
                value_end += 1;
        ...
        if after_brace < chars.len()
            && (chars[after_brace].is_ascii_digit()
                || chars[after_brace] == 'x'
                || chars[after_brace] == 'X')
        {
    ...
    if current_pos < chars.len() && chars[current_pos] == '#' {
        ...
        let error_token: String = chars[current_pos..error_end]
            .iter()
            .collect::<String>()
            .trim()
            .to_string();
        let error_token = if error_token.is_empty() {
            "#".to_string()
        } else {
            error_token
        };

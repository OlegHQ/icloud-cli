...
mod tests {
    ...
    async fn test_grep_no_newline_at_end() {
        let ctx =
            make_ctx_with_files(vec!["test", "/test.txt"], vec![("/test.txt", "test line")]).await;
        let cmd = GrepCommand;
        let result = cmd.execute(ctx).await;

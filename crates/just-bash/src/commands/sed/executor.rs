...
use super::regex_utils::{bre_to_ere, escape_for_list, normalize_for_rust};
use super::types::{
    AddressRange, ExecuteContext, PendingFileRead, PendingFileWrite, RangeState, SedAddress,
    SedCmd, SedState,
};
...
fn serialize_range(range: &AddressRange) -> String {
    ...
    format!(
        "{},{}",
        serialize_addr(&range.start),
        serialize_addr(&range.end)
    )
}
        ...
        SedCmd::Substitute {
            ...
            if has_match {
                ...
                if let Some(nth) = nth_occurrence {
                    ...
                    state.pattern_space =
                        global_replace(&state.pattern_space, &re, |matched, groups| {
                            process_replacement(&replacement, matched, groups)
                        });
                } else {
                    // Single replacement
                    let ps = state.pattern_space.clone();
                    if let Some(caps) = re.captures(&ps) {
                        ...
                        result.push_str(&process_replacement(&replacement, m.as_str(), &groups));
        ...
        if let SedCmd::Next { address } = cmd {
            ...
            if is_in_range(
                ...
                if next_idx < ctx.lines.len() {
                    ...
                    let next_line =
                        ctx.lines[ctx.current_line_index + state.lines_consumed_in_cycle].clone();
                    state.pattern_space = next_line;
                    state.line_number = ctx.current_line_index + state.lines_consumed_in_cycle + 1;

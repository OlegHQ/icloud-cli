...
pub fn builtin_length(args: &[String], ctx: &AwkContext) -> String {
    let s = if args.is_empty() { &ctx.line } else { &args[0] };
...
pub fn builtin_split(s: &str, sep: Option<&str>, fs: &str) -> (usize, HashMap<String, String>) {
...
pub fn builtin_sub(pattern: &str, replacement: &str, target: &str) -> (String, String) {
    ...
    if let Some(m) = regex.find(target) {
        ...
        let new_target = format!("{}{}{}", &target[..m.start()], new_text, &target[m.end()..]);
...
pub fn builtin_gsub(pattern: &str, replacement: &str, target: &str) -> (String, String) {
    let regex = match Regex::new(pattern) {
        Ok(re) => re,
        Err(_) => return ("0".to_string(), target.to_string()),
    };
...
pub fn builtin_gensub(pattern: &str, replacement: &str, how: &str, target: &str) -> String {
    let regex = match Regex::new(pattern) {
        Ok(re) => re,
...
pub fn call_builtin(name: &str, args: &[String], ctx: &mut AwkContext) -> Option<BuiltinResult> {
    match name {

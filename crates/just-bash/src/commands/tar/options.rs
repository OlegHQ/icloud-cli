...
fn detect_gzip_from_extension(filename: &str) -> bool {
    ...
    lower.ends_with(".tar.gz") || lower.ends_with(".tgz") || lower.ends_with(".taz")
}
...
fn set_value_option(opts: &mut TarOptions, c: char, value: String) {
    match c {
...
fn apply_short_flag(opts: &mut TarOptions, c: char) -> Result<(), String> {
    match c {
        ...
        _ => {
            return Err(format!("tar: unrecognized option '-{}'\n", c));
        }
    }
    Ok(())
}
...
pub fn parse_options(args: &[String]) -> Result<TarOptions, String> {
    ...
    while i < args.len() {
            ...
            while j < chars.len() {
                ...
                if is_value_option(c) {
                    ...
                    if j < chars.len() - 1 {
                        ...
                        i += 1;
                        if i >= args.len() {
                            return Err(format!("tar: option requires an argument -- '{}'\n", c));
                        }
                        set_value_option(&mut opts, c, args[i].clone());
                    }
                } else {
        ...
        match arg.as_str() {
            "-c" | "--create" => opts.operation = Some(TarOperation::Create),
            "-x" | "--extract" | "--get" => opts.operation = Some(TarOperation::Extract),
            "-t" | "--list" => opts.operation = Some(TarOperation::List),
            "-r" | "--append" => opts.operation = Some(TarOperation::Append),
            "-u" | "--update" => opts.operation = Some(TarOperation::Update),
            "-z" | "--gzip" | "--gunzip" => opts.gzip = true,
            "-a" | "--auto-compress" => opts.auto_compress = true,
            "-v" | "--verbose" => opts.verbose = true,
            "-O" | "--to-stdout" => opts.to_stdout = true,
            "-k" | "--keep-old-files" => opts.keep_old_files = true,
            "-m" | "--touch" => opts.touch = true,
            "-p" | "--preserve" | "--preserve-permissions" => opts.preserve = true,
            "--wildcards" => opts.wildcards = true,
            "-f" | "--file" => {
                i += 1;
                if i >= args.len() {
                    return Err("tar: option requires an argument -- 'f'\n".to_string());
                }
            ...
            "-C" | "--directory" => {
                i += 1;
                if i >= args.len() {
                    return Err("tar: option requires an argument -- 'C'\n".to_string());
                }
                opts.directory = Some(args[i].clone());
            }
            s if s.starts_with("--directory=") => {
                opts.directory = Some(s["--directory=".len()..].to_string());
            }
            s if s.starts_with("--strip-components=") || s.starts_with("--strip=") => {
                ...
                let num: usize = val
                    .parse()
                    .map_err(|_| format!("tar: invalid number for --strip: '{}'\n", val))?;
                opts.strip = num;
            }
            ...
            "--exclude" => {
                i += 1;
                if i >= args.len() {
                    return Err("tar: option '--exclude' requires an argument\n".to_string());
                }
            ...
            "-T" | "--files-from" => {
                i += 1;
                if i >= args.len() {
                    return Err("tar: option requires an argument -- 'T'\n".to_string());
                }
                opts.files_from = Some(args[i].clone());
            }
            s if s.starts_with("--files-from=") => {
                opts.files_from = Some(s["--files-from=".len()..].to_string());
            }
            "-X" | "--exclude-from" => {
                i += 1;
                if i >= args.len() {
                    return Err("tar: option requires an argument -- 'X'\n".to_string());
                }
                opts.exclude_from = Some(args[i].clone());
            }
            s if s.starts_with("--exclude-from=") => {
                opts.exclude_from = Some(s["--exclude-from=".len()..].to_string());
            }
            "--" => {
...
mod tests {
    ...
    #[test]
    fn test_parse_cvzf() {
        let opts = parse_options(&args(&["-cvzf", "archive.tar.gz", "dir/"])).unwrap();
    ...
    #[test]
    fn test_parse_xf() {
        let opts = parse_options(&args(&["-xf", "archive.tar"])).unwrap();
    ...
    #[test]
    fn test_parse_tf() {
        let opts = parse_options(&args(&["-tf", "archive.tar"])).unwrap();
    ...
    fn test_parse_long_options() {
        let opts = parse_options(&args(&["--create", "--gzip", "--file=archive.tar.gz"])).unwrap();
        assert_eq!(opts.operation, Some(TarOperation::Create));
        assert!(opts.gzip);
        assert_eq!(opts.file, Some("archive.tar.gz".to_string()));
    }
    ...
    fn test_parse_directory_option() {
        let opts = parse_options(&args(&["-C", "/tmp", "-xf", "archive.tar"])).unwrap();
    ...
    fn test_parse_exclude() {
        let opts =
            parse_options(&args(&["--exclude=*.log", "-cf", "archive.tar", "dir/"])).unwrap();
    ...
    fn test_parse_strip_components() {
        let opts = parse_options(&args(&["--strip-components=2", "-xf", "archive.tar"])).unwrap();
    ...
    fn test_combined_short_cvf() {
        let opts = parse_options(&args(&["-cvf", "archive.tar", "a", "b"])).unwrap();
        assert_eq!(opts.operation, Some(TarOperation::Create));
        assert!(opts.verbose);
        assert_eq!(opts.file, Some("archive.tar".to_string()));
    ...
    fn test_missing_operation_is_none() {
        let opts = parse_options(&args(&["-f", "archive.tar"])).unwrap();
        assert_eq!(opts.operation, None);
    }

    #[test]
    fn test_auto_compress_from_extension() {
        let opts = parse_options(&args(&["-acf", "archive.tar.gz", "dir/"])).unwrap();
    ...
    fn test_auto_compress_tgz() {
        let opts = parse_options(&args(&["-a", "-cf", "archive.tgz", "dir/"])).unwrap();
        assert!(opts.auto_compress);
        assert!(opts.gzip);
    }
    ...
    fn test_inline_f_value() {
        let opts = parse_options(&args(&["-cfmy_archive.tar", "dir/"])).unwrap();
        assert_eq!(opts.operation, Some(TarOperation::Create));
        assert_eq!(opts.file, Some("my_archive.tar".to_string()));
    }

    #[test]
    fn test_double_dash_stops_options() {
        let opts = parse_options(&args(&["-cf", "archive.tar", "--", "-weird-file"])).unwrap();
    ...
    fn test_wildcards_flag() {
        let opts = parse_options(&args(&["--wildcards", "-xf", "archive.tar"])).unwrap();
        assert!(opts.wildcards);
    }

    #[test]
    fn test_preserve_permissions() {
        let opts = parse_options(&args(&["--preserve-permissions", "-xf", "archive.tar"])).unwrap();
    ...
    fn test_exclude_separate_arg() {
        let opts =
            parse_options(&args(&["--exclude", "*.tmp", "-cf", "archive.tar", "dir/"])).unwrap();
        assert_eq!(opts.exclude, vec!["*.tmp"]);
    }

    #[test]
    fn test_files_from_option() {
        let opts = parse_options(&args(&["-T", "filelist.txt", "-cf", "archive.tar"])).unwrap();
        assert_eq!(opts.files_from, Some("filelist.txt".to_string()));
    }

    #[test]
    fn test_exclude_from_option() {
        let opts = parse_options(&args(&["-X", "excludes.txt", "-cf", "archive.tar"])).unwrap();
        assert_eq!(opts.exclude_from, Some("excludes.txt".to_string()));
    }

    #[test]
    fn test_long_directory_equals() {
        let opts = parse_options(&args(&["--directory=/tmp", "-xf", "archive.tar"])).unwrap();
        assert_eq!(opts.directory, Some("/tmp".to_string()));
    }

    #[test]
    fn test_append_operation() {
        let opts = parse_options(&args(&["-rf", "archive.tar", "newfile"])).unwrap();
    ...
    fn test_update_operation() {
        let opts = parse_options(&args(&["-uf", "archive.tar", "file"])).unwrap();
        assert_eq!(opts.operation, Some(TarOperation::Update));
    }

    #[test]
    fn test_keep_old_files() {
        let opts = parse_options(&args(&["-xkf", "archive.tar"])).unwrap();
    ...
    #[test]
    fn test_touch_flag() {
        let opts = parse_options(&args(&["-xmf", "archive.tar"])).unwrap();
        assert!(opts.touch);
    }

    #[test]
    fn test_to_stdout_flag() {
        let opts = parse_options(&args(&["-xOf", "archive.tar"])).unwrap();
